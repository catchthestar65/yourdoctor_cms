...
## Phase 6：子テーマ修正・サイト動線（最終フェーズ）

### 事前準備

- Phase 1〜5 が動作確認済み
- 監修者を設定済みの記事と未設定の記事が両方ある（差分確認用）

### デプロイ手順

main マージ → 自動デプロイ。差分は以下：
- `plugin/yd-supervisor/yd-supervisor.php`（バージョン 0.6.0、article-supervisor 読込）
- `plugin/yd-supervisor/inc/class-article-supervisor.php`（新規・the_content フィルター）
- `plugin/yd-supervisor/inc/helpers.php`（`yd_render_supervisor_cards()` 追加）
- `plugin/yd-supervisor/assets/css/article-supervisor.css`（新規）
- `theme/swell_child_custmon/header.php`（ナビに「監修医師」追加）

### 検証項目

#### A. 記事下に監修者カードが表示される

監修者を設定した記事をフロントエンドで開く：

- [ ] 本文末尾に「**この記事の監修医師**」見出しの白いカードボックスが出る
- [ ] その中に監修医師カード（顔写真円形 80px / 敬称+氏名 / 役職 / クリニック / 「プロフィールを見る →」）が表示される
- [ ] カードクリックで `/media/doctor/{slug}/` に遷移する
- [ ] 複数監修者の場合、縦に並ぶ
- [ ] レスポンシブ：スマホで顔写真 64px に縮小、レイアウト維持

#### B. 監修者ゼロの記事は何も出ない

監修者を設定していない記事をフロントエンドで開く：

- [ ] 「この記事の監修医師」セクションが**出現しない**
- [ ] `article-supervisor.css` も読み込まれない（Network タブ確認）

#### C. ヘッダーナビに「監修医師」リンクが追加される

`/media/` 配下のどのページでも：

- [ ] ヘッダーナビに「**監修医師**」項目が出現（「ドクターの素顔」の右隣）
- [ ] クリックで `/media/doctor/`（監修医師アーカイブ）に遷移
- [ ] レスポンシブ：1080px 以下でフォント縮小しても収まる

#### D. プラグイン無効化時の挙動

**重要**：プラグイン無効化シミュレーション（管理画面 → プラグイン → YD Supervisor → 無効化）

- [ ] サイトが落ちない（白画面・致命的エラーなし）
- [ ] ヘッダーナビから「監修医師」項目が**消える**（条件分岐が効いている）
- [ ] 既存記事の表示は監修カードが消えるだけで他は崩れない
...
- [ ] HTML の本文末尾に監修者カードが表示される（Phase 6）
- [ ] かつ JSON-LD で MedicalWebPage / reviewedBy が出力されている（Phase 5）
- [ ] 両者の医師名・役職が一致している

#### F. 既存ページへの副作用最終確認

- [ ] トップページ表示が崩れていない
- [ ] 既存記事ページの本文・サイドバー・フッターが従来通り
- [ ] 監修医師シングル/アーカイブも引き続き動作（Phase 3 機能）
- [ ] ターゲットKW 入力欄も引き続き動作（Phase 4 機能）

### 想定される表示崩れと対応

- **SWELL の関連記事ブロックと監修者カードの順序**：`the_content` フィルターは関連記事より前に挿入される。順序を変えたい場合は priority 調整で対応可
- **目次プラグインで H2 が拾われる**：`yd-article-supervisors__heading` の `<h2>` が目次に「この記事の監修医師」として現れる場合あり。気になる場合は `<h2>` を `<div>` に変えるカスタマイズ可

### Phase 6 完了 = 全フェーズ完了

設計書 §9 の Phase 1〜6 全て実装済み。

### 残タスク（任意）

- パーマリンク再生成（記事の slug 変更等した場合）
- Application Password 発行（前中氏の REST API 連携を始める時）
- Google Search Console での MedicalWebPage 認識状況のモニタリング
- 顔写真未設定の医師に画像追加
