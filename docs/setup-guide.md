...
## STEP 9：（オプション）GitHub Actions 自動デプロイの設定

開発が安定してきた段階で導入を推奨。

### 9-1. FTP認証情報を Repository Secrets に登録

GitHubリポジトリ → Settings → Secrets and variables → Actions → 「New repository secret」

| Secret名 | 値 |
|---|---|
| `FTP_SERVER` | FTPサーバーホスト（例：`ftp.your-doctor.jp`） |
| `FTP_USERNAME` | FTPユーザー名 |
| `FTP_PASSWORD` | FTPパスワード |

### 9-2. ワークフローを有効化

`.github/workflows/deploy.yml` を編集し、コメントアウトを外して `git push`。
詳細は同ファイル内のコメント参照。
